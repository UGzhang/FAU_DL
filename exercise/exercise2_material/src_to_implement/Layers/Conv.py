...
class Conv(BaseLayer):
    ...
    def __init__(self, stride_shape, convolution_shape, num_kernels):
        ...
        self.s_h = stride_shape[0]
        self.s_w = stride_shape[1] if type(stride_shape) == tuple else 1
    ...
    def forward(self, input_pad):
        ...
        if len(input_pad.shape) == 3:
            self._Conv1D = True
            input_pad = np.expand_dims(input_pad, axis=3)

        self._input_shape = input_pad.shape

        _, _, filter_h, filter_w = self.weights.shape
        ...
        input_pad = np.pad(input_pad, ((0, 0), (0, 0), (pad_h1, pad_h2), (pad_w1, pad_w2)), 'constant', constant_values=0)

        N, C, H, W = input_pad.shape

        out_h = int((H - filter_h) / self.s_h) + 1
        out_w = int((W - filter_w) / self.s_w) + 1

        self.input_col = Tool().im2col(input_pad, [filter_h, filter_w], [self.s_h, self.s_w])
    ...
    def backward(self, error_tensor):
        ...
        dx = Tool().col2im(dcol, self._input_shape, [filter_h,filter_w], [self.s_h, self.s_w])
